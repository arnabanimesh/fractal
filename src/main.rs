...
const BUF_SIZE: usize = WIDTH * HEIGHT * 3;
const NB_SAMPLES: usize = 50;
...
async fn main() {
    let blocking_task = tokio::spawn(async {
        ...
        while let Some(res) = rx.recv().await {
            ...
            print!("Progress: {}%\r", percentage_finished as usize);
...
fn write_line(buf: &mut Vec<u8>, line: &Vec<u8>, line_number: usize) {
    for i in 0..WIDTH {
...
fn render_line(line_number: usize, px: f64, py: f64) -> (Vec<u8>, usize) {
    let mut rng = thread_rng();

    let mut line: Vec<u8> = Vec::with_capacity(LINE_SIZE);
...
fn paint(r: f64, n: usize) -> (u8, u8, u8) {
    if r > 4. {
...
fn mandelbrot_iter(px: f64, py: f64) -> (f64, usize) {
